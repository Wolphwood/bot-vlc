...
export async function SaveUrlToLocal(url, folder, name) {
  try {
    ...
    // Check du type MIME via les headers
    const mimeType = response.headers.get('content-type');
    if (!mimeType?.startsWith('image/')) {
      throw new Error('Le contenu n\'est pas une image valide.');
    }

    // On s'assure que le dossier existe (Vanilla fs)
    if (!fs.existsSync(folder)) {
      fs.mkdirSync(folder, { recursive: true });
    }

    // Construction du nom final avec l'extension du type MIME
    const ext = mimeType.split('/')[1].split('+')[0]; // ex: image/png -> png
    const filename = `${name}.${ext}`;
    const fullPath = path.join(folder, filename);

    // Streaming direct du body vers le fichier
    // On convertit le web stream en node stream pour fs.createWriteStream
